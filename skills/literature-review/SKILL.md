...
5. Check for an active review and determine the resume point:
   ...
   - `"unproven": true` → **STOP.** Tell the user that the review's folder exists at the printed `workdir` but its ownership cannot be proven, and give the printed `error` verbatim. Once the pointer is gone, the folder is listed under `stranded`. To detach the review, the user deletes `reviews/.active-review` by hand.
