...
def _step_bash(anchor, workdir):
    """The first bash code block after `anchor` in Phase 6, for review `p`
    working in `workdir` (an absolute path, as `[workdir]` always is)."""
    tail = PHASE_6[PHASE_6.index(anchor):]
    block = re.search(FENCE + r"bash\n(.*?)" + FENCE, tail, re.S)
    return block.group(1).replace("[workdir]", str(workdir)).replace("[project-name]", "p")
...
def test_step_7_as_written_writes_the_three_files(tmp_path):
    ...
    r = subprocess.run(["bash", "-c", _step_bash("Split the delivery into its three files", rd)],
...
def test_the_safety_net_keeps_the_three_deliverables(tmp_path):
    ...
    script = _step_bash("Safety net", rd)
