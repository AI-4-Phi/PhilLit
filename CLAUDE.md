...
- `.claude/hooks/` — Git/Claude hooks: `bib_validator.py`, `validate_bib_write.py`, `metadata_validator.py`, `metadata_cleaner.py`, `block_background_bash.py`, `subagent_stop_bib.sh`, `setup-environment.sh`, `check-updates.sh` (SessionStart upstream-update notification).
...
- `docs/superpowers/` — Spec and plan files generated by the superpowers workflow (`plans/`, `specs/`). These document intended work and may or may not be implemented; treat as design history, not a source of truth for current behavior. Untracked (local only).
...
- **Hook placement**: Project-level hooks (in `settings.json`) fire for subagent tool calls. Agent frontmatter hooks ALSO fire for Task-spawned subagents in current Claude Code (this changed in 2026) — but they are ignored for plugin subagents, so PhilLit deliberately keeps all hooks in `settings.json` (single source of truth, plugin-compatible; see `docs/claude-code-deferred-opportunities.md`). Use SubagentStop hooks for validating subagent final output; all SubagentStop decisions must be stdout JSON with exit 0 (JSON is ignored on exit 2).
