...
- `hooks/` — Hook scripts: `fast_gate.sh` (shell pre-filter for per-call gates), `bib_validator.py`, `validate_bib_write.py`, `metadata_cleaner.py`, `block_background_bash.py` (guards Bash-tool background calls inside subagents), `block_subagent_background_dispatch.py` (guards Agent/Task background dispatch at the orchestrator — the four PhilLit review agents must run foreground), `block_ledger_write.py` (denies **native file-tool** writes to the `enrichment_ledger-*.json` / `cleaning_ledger-*.json` attestation ledgers; the designated writers write them from Python, so the pipeline is unaffected. It is **not** a security boundary — broadly-allowed `Bash` bypasses it — and it also denies a *developer's* hand edit: use the owning script or `git checkout`), `subagent_stop_bib.sh`, and the thin `setup-environment.sh` SessionStart bootstrap. Also here but **not hooks** (absent from `hooks.json`; here because other components already import from this directory): four shared modules, each the ONE owner of its concern. Read the module docstring before changing one — it carries the grammar, the rationale and the measured edge cases.
  - `bib_comments.py` — the `@comment` / `@string` / `@preamble` block grammar: `is_verbatim_block` (the one test for "is this chunk a block to carry" — consumers, dedupe included, decide it FIRST, before any entry-header regex), `comment_defects` (validator check 4c and the cleaner's refusal preflight), `stray_text`. "Line start" means column 0 everywhere, because every splitter chunks at `\n(?=@)`.
  - `cleaning_marker.py` — the METADATA_CLEANED marker GRAMMAR; the cleaner owns the FORMAT it writes.
  - `ledger_binding.py` — the ledger-to-bib content binding (`bib_sha256`). The hash covers DECODED TEXT, never bytes. The binding applies to `kind == "cleaning"` only; the enrichment ledger is exempt from the BINDING, not from version checking. Each kind is pinned to exactly the versions ITS OWN producer writes (`_CLEANING_VERSIONS` / `_ENRICHMENT_VERSIONS` in the barrier) — never merge the two sets, and never widen one to accept an older or unknown version. Bump `BINDING_SCHEMA_VERSION` only in `ledger_binding.py`. The barrier re-points an ACCEPTED ledger per domain, immediately after that domain's bib write, binding the text it AUTHORED — never a read-back, never a rejected ledger (`evidence_barrier._repoint_binding` and `execute` carry the why).
...
- **pybtex Writer emits quoted fields** (`field = "value"`) on round-trip, not just braced — the metadata cleaner's rewrite overrides this to braces, but older and hand-edited bibs still carry quotes, so anything reading `.bib` text must accept both forms, plus bare values (`year = 2016`) and braces nested to any depth (`Mendon{\c{c}}a`). Do not write a new field regex: locate fields with `bib_fields.iter_fields` / `parse_entry_fields`, the depth-counting scanner. A one-level regex silently dropped 33 delivered entries' author or title before it was replaced.
