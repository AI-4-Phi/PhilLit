...
- `skills/literature-review/` — Main orchestration skill for the 6-phase workflow. `scripts/` contains the Phase 3-to-4 evidence barrier (`evidence_barrier.py`, helpers `resolve_context.py`, `stamp_evidence.py`, `venue_vetting.py` — the OpenAlex venue check behind the barrier's `venue_status` flag — and `year_suffix.py`, the pure Chicago a/b assigner behind its `year_suffix` field) and Phase 6 tools: `assemble_review.py`, `normalize_headings.py`, `dedupe_bib.py`, `enrich_bibliography.py`, `generate_bibliography.py`, `lint_md.py`, `check_evidence.py`, `sanitize_bib.py`.
