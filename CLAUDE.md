...
- `hooks/` — Hook scripts: `fast_gate.sh` (shell pre-filter for per-call gates), `bib_validator.py`, `validate_bib_write.py`, `metadata_cleaner.py`, `block_background_bash.py` (guards Bash-tool background calls inside subagents), `block_subagent_background_dispatch.py` (guards Agent/Task background dispatch at the orchestrator — the four PhilLit review agents must run foreground), `block_ledger_write.py` (denies **native file-tool** writes to the `enrichment_ledger-*.json` / `cleaning_ledger-*.json` attestation ledgers — the designated writers write them from inside Python, so the supported pipeline is unaffected; it is **not** a security boundary, since broadly-allowed `Bash` bypasses it entirely, and it does deny a *developer* hand-editing a ledger — use the owning script or `git checkout`), `subagent_stop_bib.sh`, and the thin `setup-environment.sh` SessionStart bootstrap. Also here but **not a hook**: `bib_comments.py`, the `re`-only leaf that owns the `@comment` block GRAMMAR (`is_verbatim_block` — the one test for "is this chunk a block to carry", which dedupe decides FIRST, before its header regex, and the cleaner's `comment_blocks` bind as aliases; an entry type that merely begins with "comment" is an entry, and `@string`/`@preamble` are carried like comments — and `comment_defects`, the list behind the validator's check 4c AND the cleaner's refusal preflight: every `@` inside a `@comment` chunk after its command (pybtex's own rule, and the comment's alone — a comment runs to the next `@` whatever the braces say, so a braced `@word{...}` there is a second entry and the block's tail is silently lost; a `@string` value or `@preamble` is read whole, so an `@` inside one is ordinary text) plus `stray_text`, every run of text outside a verbatim chunk and outside its chunk's entry — a `%%` divider, a stray `}`, the tail a column-0 `@` cut off a block, an `@` command that does not start its line (a BOM or indentation before the FIRST entry drops it in dedupe), a column-0 entry without the `@type{key,` header dedupe, stamping and the field scanner all read (a `(`-delimited entry, a hyphenated type, a key with no comma), which dedupe would otherwise drop with only a warning, and a `@string`/`@preamble` that does not balance within its chunk (pybtex reads on through the lines after it as the value, entries included, until a later closer or end of file; the column-0 tools cut it at the next line-start `@`, so a value line may not start with `@`) — because a rewrite keeps only those two things and downstream agents read the bib as text. "Line start" means column 0 everywhere, because every splitter chunks at `\n(?=@)`); `cleaning_marker.py`, the `re`-only leaf that owns the METADATA_CLEANED marker GRAMMAR (`marker_removed_fields`, `marker_type_changed`, `has_marker`; the cleaner still owns the FORMAT it writes) — a separate module because `bib_validator` must read markers and `bib_identity` already imports `bib_validator`, so the cleaner cannot be imported from there; `ledger_binding.py`, the `hashlib`-only leaf that owns the ledger-to-bib CONTENT binding (`bib_text_sha256`, `bib_file_sha256`, `binding_holds`, `BINDING_SCHEMA_VERSION`) — the cleaning ledger carries a `bib_sha256` from schema 3 so a stale ledger a failed unlink left standing is rejected however it survived; the hash covers DECODED TEXT, never bytes, because the cleaner writes in text mode and Windows gets CRLF. The rule is scoped to `kind == "cleaning"`, and for that kind the version is a FLOOR — a v1/v2 cleaning ledger is refused, so there is no version a ledger can declare to skip the binding. The enrichment ledger is exempt and stays at 1: it is written at the researcher's Stage 5.5, before the cleaner rewrites the bib at SubagentStop, so any hash it took would always be of a superseded text — and the kind scoping is what stops a future enrichment-schema bump from silently opting it into a contract it cannot satisfy. Each kind is pinned to the versions ITS OWN producer writes (`_CLEANING_VERSIONS` / `_ENRICHMENT_VERSIONS`) — sparing a kind the BINDING is not sparing it VERSION checking, and a shared accepted set once let an enrichment ledger declare a version nothing implements and be read under the old one. The barrier re-points the binding on a ledger it ACCEPTED, per domain and immediately after that domain's bib write (a rejected ledger is left alone, or a re-run would launder it; batching the re-points stranded an earlier domain when a later write failed). It binds the text the barrier AUTHORED, never a read-back — `write_text`/`read_text` translate newlines symmetrically so the digests are identical even where the disk holds CRLF, and a read-back would sign whatever edited the file after the write. A refusal also records `cleaning_ledger_reason` / `enrichment_ledger_reason` in the report, because a bare `malformed` cannot tell an operator to regenerate the ledger, look for a changed bib, or suspect the platform. `binding_holds` deliberately has no shape guard in front of its compare; one was removed after no mutation could distinguish it. And `bib_identity.py`, the one owner of bibliography identity, comparison keys, and name folds (`normalize_doi`, `title_key`, `normalize_pages`, `normalize_journal`, `venue_key`, `year_key`, `fallback_key`, `same_work_key`, `same_work_year`, `ascii_variants`, `translit_fold`, `contract_fold`, `split_author_list`, `first_author_name`, `first_author_surname`, `first_author_prose_surname`) — note `venue_key` is a deliberately looser VERIFICATION key for venue names (it folds conference citation forms onto the series name) and must never be used for dedup identity, which is `normalize_journal`'s job. Author lists are split ONLY through `split_author_list` (pybtex, brace-aware, no literal fallback) — a literal `.split(" and ")` keyed a braced corporate author differently from Phase 6; `first_author_surname` keeps the brace/LaTeX spelling of pybtex's selected surname parts rather than decoding them into search text, while `enrich_bibliography.get_author_last_name` derives search text from it. There are **two** surname rules and they are not interchangeable: `first_author_surname` is IDENTITY text (pybtex prelast+last, braces kept, feeding `fallback_key` / `same_work_key`), `first_author_prose_surname` is text to search FOR in prose (the part before the first comma; `resolve_context`'s SEP match is its one consumer — `check_evidence.find_cites` searches `enrich_bibliography.get_author_last_name`'s search text instead (case-protection braces stripped, LaTeX escape groups kept, a comma-less name reduced to its last token), the switch the 2026-09-04 surname census licensed for that consumer alone). On the author-only path its consumer uses, they differ by ONE mechanism — the stripped pre-comma prefix here, pybtex's SELECTED surname parts joined by single spaces there — so they diverge when that rendering differs from the prefix (a multi-token comma-less name, an unprotected non-space separator inside the surname, a braced name with an internal comma; a PROTECTED group agrees). Two paths sit outside that: `first_author_surname`'s editor fallback, and its comma-less `Person`-failure fallback, which returns the last token. Never collapse them, and never restate the divergence as a fixed list of shapes — every attempt to enumerate it was wrong, each by generalising over sampled inputs. The owner's docstring transcribes the three branches off the code instead; read it there rather than summarising it here. It lives in `hooks/` because that is the directory other components already import from, and it is deliberately absent from `hooks.json`. Sites bind the shared objects as **aliases**, never a local copy, and tests assert `is` identity. A site-local alias may keep its historic name or take a clearer one — but it must never SHADOW a different shared object's name, which is why `resolve_context.first_author_surname` became `prose_surname`: `bib_identity` exports a `first_author_surname` implementing the other (identity) rule, and `year_suffix.py` imports it from the same directory.
...
- **pybtex Writer emits quoted fields** (`field = "value"`) on round-trip, not just braced — the metadata cleaner's rewrite overrides this to braces since 0.5.17, but bibs cleaned before then and hand-edited ones still carry quotes, so anything reading `.bib` text must accept both forms, plus bare values (`year = 2016`) and braces nested to any depth (`Mendon{\c{c}}a`). Do not write a new field regex: locate fields with `bib_fields.iter_fields` / `parse_entry_fields`, the depth-counting scanner. A one-level regex silently dropped 33 delivered entries' author or title before it was replaced.
