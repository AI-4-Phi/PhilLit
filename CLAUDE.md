...
- **pybtex Writer emits quoted fields** (`field = "value"`) on round-trip, not just braced — anything reading `.bib` text must accept both forms, plus bare values (`year = 2016`) and braces nested to any depth (`Mendon{\c{c}}a`). Do not write a new field regex: locate fields with `bib_fields.iter_fields` / `parse_entry_fields`, the depth-counting scanner. A one-level regex silently dropped 33 delivered entries' author or title before it was replaced.
