...
The guard is an invariant: the output must equal the input under a canonical projection that strips the barrier-owned fields (`keywords` `EVIDENCE-*`, `year_suffix`, `web_span`, `venue_status`, `same_work_group`, `urldate`, `archiveurl`).
