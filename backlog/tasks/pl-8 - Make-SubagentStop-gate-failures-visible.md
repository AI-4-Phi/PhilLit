---
id: PL-8
title: Make SubagentStop gate failures visible
...
**Why:** CLAUDE.md's gate-failure policy forbids an accuracy gate from failing open silently. Today a researcher's bib can skip validation and cleaning with no visible sign.
...
Ships with PL-1 (the root `.bib` sweep), which changes the same file.
