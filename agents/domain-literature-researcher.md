...
- Never write a `venue_status`, `year_suffix`, or `same_work_group` field yourself
  — the evidence
  barrier owns them and re-derives them from scratch on every run, like the
  ...
  `same_work_group` names entries that look like one work reprinted, grouped
...
- **No text outside entries and `@comment{}` blocks** — no `%` comment lines, no stray braces, every top-level `@` command at the start of its line, and every entry headed `@type{key,` (braces; no space after the type or before the key): the metadata rewrite keeps only entries and verbatim `@comment{}` / `@string` / `@preamble` blocks, and dedupe or evidence stamping drops or misses an entry with any other header, so the validator rejects what they would lose rather than let that happen
