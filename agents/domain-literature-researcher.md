...
3. **Use enriched metadata**: Prefer CrossRef's `container_title`, `volume`, `issue`, `page` over S2/OpenAlex fields. For a chapter, `container_title` is the book (`booktitle`); when the output also carries a non-empty `series`, include it as the entry's `series` field
