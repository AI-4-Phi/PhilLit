...
This document records improvement opportunities identified in the audit that were deliberately **not** acted on, plus one parked design (§2).

---

## 1. Bash sandboxing as the safety layer (medium, platform-constrained)

Today, autopilot safety with the global `Bash` allow rests on three Bash deny rules (`sudo`, `dd`, `mkfs`) and two ask rules (`rm`, `rmdir`). Native sandboxing enforces OS-level guarantees instead:
...
## 2. Dynamic-workflow orchestration for Phases 3–5 (parked)
...
### 3.6 PreToolUse `updatedInput` (auto-fix instead of deny)
