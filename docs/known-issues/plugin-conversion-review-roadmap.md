...
- **Phase 2 — harden + re-run the parity gate**: finding 7. Re-test from a plugin copy *outside* the repo, with a workspace `.env` that differs from the shell env, and an explicit hook-process check. Status: **pending**.
